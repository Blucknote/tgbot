...
assert f['result'] == True, "Webhook fail, %s" % f

f=tgapi.send_message(106989752, 'test')
assert f['result'] == True, "sending message fail, %s" % f
...
assert f['result'] == True, "sending image fail, %s" % f
img.close()

vid = open('test.mp4', 'rb')
f=tgapi.send_photo(106989752, vid.read(), 'test')
assert f['result'] == True, "sending video fail, %s" % f
vid.close()


doc = open('test.pdf', 'rb')
f=tgapi.send_document(106989752, doc.read(), 'test')
assert f['result'] == True, "sending document fail, %s" % f
doc.close()
...
assert f['result'] == True, "retrieve updates fail, %s" % f

f=tgapi.get_me()
assert f['result'] == True, "get bot info fail, %s" % f
